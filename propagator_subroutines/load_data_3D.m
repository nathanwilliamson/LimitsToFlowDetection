function [Image,dims,fov,grad]=load_data_3D(pathname,im_ind)
...
%% unpack data    
    ...
    for j=1:num_grad
        Image_split(i-reco_ind(1)+1,:,:,:,j)=A1(i-reco_ind(1)+1,:,:,:,j)/slope(j); %un-normalizing the data

    end
    fclose(reco);
    fclose(version_hand);
end
Image_real=squeeze(Image_split(1,:,:,:,:));
Image_img=squeeze(Image_split(2,:,:,:,:));
