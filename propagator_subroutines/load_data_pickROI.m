function [Image_comp_s1,Image_comp_s2,Image_roi_s1,Image_roi_s2,bw_mat,dims,fov,mean_roi_s1,mean_roi_s2,grad]=load_data_pickROI(pathname,im_ind)
...
for i=reco_ind
    ...
    slope2=cellfun(@str2num,[slope_cell{:}]);
...
%         A1_s2(i-reco_ind(1)+1,:,:,:)=reshape(A_s2,[dims(1),dims(2),num_grad]);
        ...
        A1_s2(i-reco_ind(1)+1,:,:,:)=A1(i-reco_ind(1)+1,2,:,:,:);

    
    for j=1:num_grad
        Image_s1(i-reco_ind(1)+1,:,:,j)=A1_s1(i-reco_ind(1)+1,:,:,j)/slope(j); %un-normalizing the data
        Image_s2(i-reco_ind(1)+1,:,:,j)=A1_s2(i-reco_ind(1)+1,:,:,j)/slope(j);
    end
...
Image_real_s2=squeeze((Image_s2(1,:,:,:)));
Image_img_s2=squeeze((Image_s2(2,:,:,:)));
Image_comp_s2(1,:,:,:)=complex(Image_real_s2,Image_img_s2);
Image_mag_s2=abs(squeeze(Image_comp_s2));
...
Image_real_s2=squeeze((Image_s2(1,:,:,:)));
Image_img_s2=squeeze((Image_s2(2,:,:,:)));
Image_comp_s2(k,:,:,:)=complex(Image_real_s2,Image_img_s2);
...
for k=1:length(im_ind)
    for i=1:num_grad
        ...
        Image_roi_s2(k,:,:,i)=squeeze(Image_comp_s2(k,:,:,i)).*bw_mat;
        tmp2=squeeze(Image_roi_s2(k,:,:,i));
        roi_s2{k,i}=tmp2(tmp2~=0);
    end
end
...
for k=1:length(im_ind)
    for i=1:num_grad
        ...
        mean_roi_s2(i)=mean(mean(roi_s2{k,i}));
        Image_cut_s2(k,i,:)=roi_s2{k,i};
    end
end
